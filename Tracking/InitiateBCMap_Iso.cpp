...
UINT4 Tracking::InitiateBCMap_Iso(ifstream &ifHandle, grid &BCMap, Atmosphere &atm){
  ...
  UINT4 nzones = atm.getnzones();
  
  UINT4 gridsize = atm.getSortedGrid().size();
  ...
  try{
    ...
    if(nZns-nzones < 0){
      cout << "FATAL ERROR: there are " << nzones << " counted in the map and only " << nZns << "in the boundary dataset" << endl;
      throw(EXIT_FAILURE);
    }
    ...
    for (UINT4 a = 0; a < nZns; a++ ){
      for (UINT4 i = 0; i < gridsize; i++ ){

	if(atm.getSortedGrid()[i].zone == Zns[a])
	  {
	    ...
	    for (unsigned int j = 0; j < atm.getSortedGrid()[i].cells.size() ; j++)
	      {
		r = atm.getSortedGrid()[i].cells[j].row;
		c = atm.getSortedGrid()[i].cells[j].col;
		
		BCMap.matrix[r][c] = data[a];
