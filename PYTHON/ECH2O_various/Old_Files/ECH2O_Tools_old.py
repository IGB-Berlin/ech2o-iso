...
def gen_paras(Opti, Config):
    ...
    if Config.sampling=='LHS' or Config.sampling=='LHS_m':
        ...
        for i in range(Opti.nvar):
            ...
            print str(Opti.names[i]);
            print str(Opti.max[i]);
            print str(Opti.min[i]);
            if Opti.log[i]==1:                
...
def create_inputs(Opti, Paras, Site, Config, it):
    ...
    # Read each soil map unit and apply param value                
            ...
            if pname=='Porosity':
                report(outmap*0.9,Config.PATH_SPA+'/SWC.L1.map')
                report(outmap*0.95,Config.PATH_SPA+'/SWC.L2.map')
                report(outmap*0.95,Config.PATH_SPA+'/SWC.L3.map')        
                #report(outmap*0.8,Config.PATH_SPA+'/SWC.L1.map')
                #report(outmap*0.8,Config.PATH_SPA+'/SWC.L2.map')
                #report(outmap*0.8,Config.PATH_SPA+'/SWC.L3.map')        
...
def manage_outputs(Data, Opti, Config, it):
    ...
    for oname in Data.names:
        if Data.obs[oname]['type']!='map' and (it==0 or Opti.begfail==1):
    ...
    for oname in Data.names:
       #print oname,
        ...
        if Data.obs[oname]['type']=='map':
            ...
            if len(glob.glob(Config.PATH_OUT+'/Maps'))==0:
                os.system('mkdir '+Config.PATH_OUT+'/Maps')
            ...
            for it2 in range(1,Data.lsim+1):
                ...
                if it2 > Config.spinup and it2 >= Config.trimB and it2 < Config.trimB+Config.trimL:
                    ...
                    if Config.MapAv == 1:
                        #print MapAv
                        tloc = Config.treal[it2-Config.trimB]
                        # 1. Monthly + instatenously (output time step) values
                        if Data.obs[oname]['typeT']=='inst' and Config.MapAvT == 'month':
                            #print nday
                            if tloc.day == 1 : 
                                # first day of the month: write past sum if it exists, start new sum
                                nday = lmon(tloc.year,tloc.month)[1]
                                MapAv[oname] = readmap(f_m)/nday
                            elif oname in MapAv:
                                #print 'add next day'
                                MapAv[oname] = MapAv[oname]+readmap(f_m)/nday
                                if tloc.day == lmon(tloc.year,tloc.month)[1]:
                                    #print 'output this month'
                                    f_o =  Config.PATH_OUT+'/Maps/'+oname+'_s'+str(it+1)+'_'+str(tloc.year)+format(tloc.month,'02')+'.map'
                                    report(MapAv[oname],f_o)

                        # 2. Monthly + accumulated values: substract end-of-month value with that of the last previous-month day
                        if Data.obs[oname]['typeT']=='acc' and Config.MapAvT == 'month':
                            if tloc.day == lmon(tloc.year,tloc.month)[1] : 
                                # Last day of the month: Output if there was already a record, anyway store new value
                                if oname in MapAv:
                                    nday = lmon(tloc.year,tloc.month)[1]
                                    f_o =  Config.PATH_OUT+'/Maps/'+oname+'_s'+str(it+1)+'_'+str(tloc.year)+format(tloc.month,'02')+'.map'
                                    report((readmap(f_m)-MapAv[oname])/nday,f_o)
                                MapAv[oname] = readmap(f_m)

                        # 3. Seasonal + instan: use JFM, AMJ, JAS, OND
                        elif Data.obs[oname]['typeT']=='inst' and Config.MapAvT == 'season':
                            if tloc.day == 1 and tloc.month in [1,4,7,10]:
                                # first day of the season: write past sum if it exists, start new sum
                                nday = lmon(tloc.year,tloc.month)[1]+lmon(tloc.year,tloc.month+1)[1]+lmon(tloc.year,tloc.month+2)[1]
                                #print 'store first day of season',
                                MapAv[oname] = readmap(f_m)/nday
                            elif oname in MapAv:
                                #print 'add day',tloc.day,
                                MapAv[oname] = MapAv[oname]+readmap(f_m)/nday
                                if tloc.day == lmon(tloc.year,tloc.month)[1] and tloc.month in [3,6,9,12]:
                                    # Last day of the season: output (if there was already a record)
                                    #print 'output season (month'+str(tloc.month-2)+'-'+str(tloc.month)+')'
                                    f_o =  Config.PATH_OUT+'/Maps/'+oname+'_s'+str(it+1)+'_'+str(tloc.year)+'.'+format(tloc.month-2,'02')+'-'+format(tloc.month,'02')+'.map'
                                    report(MapAv[oname],f_o)
                            
                        # 4. Seasonal + acc: use JFM, AMJ, JAS, OND
                        elif Data.obs[oname]['typeT']=='acc' and Config.MapAvT == 'season':
                            if tloc.day == lmon(tloc.year,tloc.month)[1] and tloc.month in [3,6,9,12]:
                                # Last day of the season: Output if there was already a record, anyway store new value
                                if oname in MapAv:
                                    nday = lmon(tloc.year,tloc.month)[1]+lmon(tloc.year,tloc.month-1)[1]+lmon(tloc.year,tloc.month-2)[1]
                                    #print 'output season (month'+str(tloc.month)+')'
                                    f_o =  Config.PATH_OUT+'/Maps/'+oname+'_s'+str(it+1)+'_'+str(tloc.year)+'.'+format(tloc.month-2,'02')+'-'+format(tloc.month,'02')+'.map'
                                    report((readmap(f_m)-MapAv[oname])/nday,f_o)
                                #print 'store last day of season',
                                MapAv[oname] = readmap(f_m)
                        
                        # 5. Otherwise, error
                        else:
                            sys.exit('Error: invalid period for map averging + map typeT')
                            
                    # No temporal average
                    else:
                        f_o = Config.PATH_OUT+'/Maps/'+oname+'_s'+str(it+1)+'_t'+suf2
                        os.system('mv '+f_m+' '+f_o)

                # Not in the targeted time range
                else:
                    os.system('rm -f '+f_m)
            print
...
def morris_trajs(Config, Opti):
    ...
    Opti.Bnorm = np.zeros((Opti.nvar,Opti.nvar+1,Opti.nr),np.float64) # the normalized
    Opti.Bstar = np.zeros((Opti.nvar,Opti.nvar+1,Opti.nr),np.float64) # the final used in runs
    ...
    for ir in range(Opti.nr):
        for iv in range(Opti.nvar):
            ...
            if(Config.MSspace=='trajectory'):
                ...
                Opti.Bnorm[:,iv+1,ir] = Opti.Bnorm[:,iv,ir] 
            elif(Config.MSspace=='radial'):
                # alway start from initial
                Opti.Bnorm[:,iv+1,ir] = Opti.Bnorm[:,0,ir] 
            else:
...
def morris_ee(Config, Data, Opti):
    ...
    for oname in Data.names:
        ...
        if Data.obs[oname]['type'] != 'map':
            ...
            bias = np.zeros((Opti.nvar),np.float64)*np.nan
            RMSE = np.zeros((Opti.nvar),np.float64)*np.nan
            for i in range(Opti.nvar):
                bias[i] = np.mean(sim[i+1,:] - sim[i,:])
                RMSE[i] = np.sqrt(np.mean((sim[i+1,:] - sim[i,:])**2))
            bias_ee = bias / Opti.stepN
            RMSE_ee = RMSE / Opti.stepN
