...
def create_inputs(Opti, Paras, Site, Config, it):
    ...
    # Read each soil map unit and apply param value                
            ...
            # No spatial/veg dependence, but channel stuff
                ...
                elif Paras.ref[pname]['file'] == 'chanwidth':
                    outmap = Site.bmaps['chanmask']*Opti.x[Paras.ind[pname]]
                else:
        ...
        if Data.obs[oname]['type']=='map':
            ...
            for it2 in range(1,Data.lsim+1):
                ...
                if it2 > Config.spinup and it2 >= Config.trimB and it2 < Config.trimB+Config.trimL:
                    ...
                    if it2>=5000:
                        suf = ''.join(list(np.repeat('0',lensuf-1)))+'4.'+format(it2-5000,'03')
                    if it2>=6000:
                        suf = ''.join(list(np.repeat('0',lensuf-1)))+'4.'+format(it2-6000,'03')
                    if it2>=7000:
                        suf = ''.join(list(np.repeat('0',lensuf-1)))+'4.'+format(it2-7000,'03')
                    if it2>=8000:
                        suf = ''.join(list(np.repeat('0',lensuf-1)))+'4.'+format(it2-8000,'03')
                    if it2>=9000:
                        suf = ''.join(list(np.repeat('0',lensuf-1)))+'4.'+format(it2-9000,'03')
