       ...
       #print oname,
        ...
        if Data.obs[oname]['type']=='mapTs':
            ...
            if Config.MapAv == 0:
                ...
                # Need to adjust the time-series if the average is in weeks
                if Config.MapAvT == "week":
#                    var_t = np.array([(Config.treal[x-Config.trimB]-datetime(1901,1,1,0,0)).days for x in itOK])
                # Need to adjust the time-series if the average is in months
                elif Config.MapAvT == 'month':
#                    var_t = np.array([(Config.treal[x-Config.trimB]-datetime(1901,1,1,0,0)).days for x in itOK])
                # Need to adjust the time-series if the average is in seasons
                elif Config.MapAvT == 'season':
#                    var_t = np.array([(Config.treal[x-Config.trimB]-datetime(1901,1,1,0,0)).days for x in itOK])
