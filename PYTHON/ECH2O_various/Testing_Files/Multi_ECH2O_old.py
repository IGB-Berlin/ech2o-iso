...
parser.add_option("--mode",dest="mode",metavar="mode",help="Switch ('calib_sampling','calib_runs','forwards_runs','sensi_morris')")

# Configuration file
parser.add_option("--file",dest="file",metavar="FILE",help="Name of the file that defines the calibration to perform")
...
parser.add_option("--MapAv",dest="MapAv",metavar="MapAv",help="0 or 1, to temporally average spatial outputs)")
parser.add_option("--MapAvT",dest="MapAvT",metavar="MapAvT",help="Averaging period (for now only monthly is available)")

# If mode == 'sensi_morris'
# Only generating trajectories ?
parser.add_option("--MSinit",dest="MSinit",metavar="MSinit",help="Switch for Morris sensitivity: 1=trajectories generation, 0=runs")
parser.add_option("--MSspace",dest="MSspace",metavar="MSspace",help="Walk in the parameter space for Morris: 'trajectory' or 'radial'")
...
if options.MapAv != None:
    ...
    if Config.MapAv == 1:
        if options.MapAvT in ['month','season']:
...
if options.Resol != None:
    Config.Resol = int(options.Resol)
else:
    Config.Resol = 30
...
if Config.runECH2O == 1:
    ...
    Config.PATH_SPA_REF = os.path.abspath(os.path.join(Config.PATH_MAIN,'Spatial_'+str(Config.Resol)+'m'))
    ...
    os.system('cp -pr '+Config.PATH_SPA_REF+'/*.map '+Config.PATH_SPA)
    os.system('cp -pr '+Config.PATH_SPA_REF+'/SpeciesParams.tab '+Config.PATH_SPA)
    ...
    with open(os.path.join(Config.PATH_OUT, cfg_ech2o),'a') as fw:
        ...
        fw.write('ClimateZones = ClimZones_'+str(Config.Resol)+'m.map\n')
        fw.write('Isohyet_map = isohyet_'+str(Config.Resol)+'m.map\n')
        if Config.isTrck == 1:
            fw.write('Tracking = 1\n')
...
if Config.mode == 'calib_sampling':
    ...
    print
    print str(Opti.nsamptot)+' sets of parameters will be generated...'
