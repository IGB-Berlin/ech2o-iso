...
# Climate Folder name
parser.add_option("--Clim",dest="Clim",metavar="Clim",help="Climate directory name, by default Climate")
...
parser.add_option("--itres",dest="itres",metavar="itres",help="Specify next iteration")
parser.add_option("--sas",dest="sas",metavar="sas",help="Option for stepwise concentration addition")
...
if options.restart == None:
    ...
    if options.itres == None:
        sys.exit("Choose which restart values")
    else:
        Config.itres = int(options.itres)
...
# -- Climate directory (default Climate)
if options.Clim == None:
    Config.Clim = 'Climate'
else:
    Config.Clim = copy.copy(options.Clim)
...
#    print(Config.Clim)
    Config.PATH_CLIM_REF = os.path.abspath(os.path.join(Config.PATH_MAIN,'../'+Config.Clim)) #Climate'))
    Config.PATH_CLIM_REF_P = os.path.abspath(os.path.join(Config.PATH_MAIN,'../'+Config.Clim+'/Steps'))
...
if options.trimL == None:
    Config.trimL = (Data.lbeg + Data.lsim) - Config.trimB + 1
else:
    if int(options.trimL) <= (Data.lbeg + Data.lsim) - Config.trimB+1:
...
if Opti.CreatePlot == None:
    Opti.CreatePlot = 0    

if int(Opti.nit) > 1:
    Opti.CreatePlot = 0
    print(Opti.CreatePlot)
...
if Config.runECH2O == 1:
    ...
    print("")
    print("EcH2O output file names")
    print(Data.names)
    Data.meas_names = sorted(Data.meas.keys())
    NumEval = 0
    ...
    print("")
    print("Efficiency criteria file names")
    print(Data.meas_names)
    print("")
...
#        print('SAS option')
#        print(options.sas)
        if int(options.sas) == 1:
#            print(Config.PATH_CLIM_REF_P+'/d2H_'+str(it+1)+'.txt')
...
#                start_time = time.time()
                ECH2O_Tools.efficiency_criteria(Data,Opti,Config,it)
#                print("--%s seconds Efficiency ----" % (time.time() - start_time))
            ...
            if Config.repBS == 1:
                ...
                os.system('mv '+Config.PATH_EXEC+'/Basind2HSummary.txt '+Config.PATH_OUT+'/Basind2HSummary_run'+str(it+1)+'.txt')
        os.chdir(Config.PATH_OUT)
        #sys.exit()
