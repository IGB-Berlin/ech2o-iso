...
if switch == 1:
    ...
    # Metrics used
    metric = ['total','stream','satext','SWC','T']
    nmet = len(metric)
    ...
    Jmse = np.genfromtxt(f_in,delimiter=',',skip_header=1,unpack=True)[nobs+1]
    ...
    # using transpiration
    JT = np.genfromtxt(f_in,delimiter=',',skip_header=1,unpack=True)[8]
    #print Jmse
    #print Jns
    ...
    for im in range(nmet):
        # Using the MSE J
        if im==0:
            print '   -Using total cost function...'
            Jsort = Jmse
        if im==1:
