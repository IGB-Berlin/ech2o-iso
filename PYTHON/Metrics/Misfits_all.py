...
obsnames = ['Streamflow','SaturationArea','SWC_Peat','SWC_Gley','SWC_Podzol',
            'SWC_NFforest','SFforest','T_NFforest']#,'SWC_forest','T_forest']
nobs = len(obsnames)
# Corresponding simulations outputs
simdir = os.getcwd()+'/'+MCname+'_sampling.'
simfiles = ['Streamflow.txt','SaturationArea.tab',
            'SoilMoistureAv.pt2.tab','SoilMoistureAv.pt3.tab','SoilMoistureAv.pt4.tab',
            'SoilMoistureAv.pt5.tab','SoilMoistureAv.pt6.tab','Transpiration[0].pt5.tab']
# Unit conversion sim --> obs
simfct = [1,1,1,1,1,1,1,8.64e7] # sim T is in m/s --> mm/d
# Obs filenames
...
fitend = [datetime(2014,10,31),datetime(2014,9,30),datetime(2014,10,31),
          datetime(2014,10,31),datetime(2014,10,31),datetime(2016,2,24),
          datetime(2014,10,31),datetime(2015,9,28)]

# Length of the simulated outputs
simlen = 1827
simdate = [datetime(2011,6,1)+timedelta(days=x) for x in range(simlen)]
