...
import time
import sys
sys.path.append( "C:\PLATO\Bruntland\STARRmodel" )
import initialPara_BB
import reporting_BB

# model modules, located in directory C:\PLATO\STARR
sys.path.append( "C:\PLATO\STARR" )
#import snowModule
import snowEbal
import interception
import soilStorage
...
wd  ='C:\PLATO\Bruntland\STARRmodel'
os.chdir(wd)

# remove prior monte carlo output folders to allow new model run 
...
for y in numdirs:                                                              # remove the content of these directories
    shutil.rmtree(wd+"\\"+y)
...
#class IsoModel(DynamicModel):
#  def __init__(self, number_of_timesteps,location_of_input_folder):
  ...
  def initial(self):
    ...
    # DECISION RULES                #
    #################################
    # Here you switch on/off bits of the model
    
    # WRITE MAP OUTPUT          ????
    self.reportSpatial        = True
    
    # WRITE TIMESERIES OUTPUT   ????
    self.reportTimeseries     = False
...
# input data for 1 June 2011 - 15 oct 2014 -> 1233 timesteps
number_of_timesteps =   1233
...
######myModel = IsoModel(number_of_timesteps, location_of_input_folder)
